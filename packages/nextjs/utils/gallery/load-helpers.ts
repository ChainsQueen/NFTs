...
/**
 * Ensures the Gallery has up to 12 visible items by synthesizing catalog entries from baseURI.
 * It does NOT assert on-chain existence; owners remain empty. Useful for a storefront-style gallery.
 *
 * @param kc Contract client with read.baseURI() and read.tokenURI() support
 * @param fetched Items discovered on-chain (minted)
 */
export async function ensureCatalog12(kc: any, fetched: GalleryItem[]): Promise<GalleryItem[]> {
  try {
    const have = new Set(fetched.map(i => i.id));
    if (have.size >= 12) return fetched;
    // Try baseURI first; fallback to inferring from tokenURI(1)
    let base = "";
    try {
      base = String(await withTimeout(kc.read.baseURI(), 8000, "baseURI()"));
    } catch {
    ...
    const targets = Array.from({ length: 12 }, (_, i) => i + 1).filter(id => !have.has(id));
    const additions = await mapWithConcurrency(targets, 6, async id => {
      try {
        const uri = `${base}${id}.json`;
        const resolved = resolveIpfsToHttp(uri);
        let meta: NFTMetaData | undefined;
    ...
    const valid = additions.filter(Boolean) as GalleryItem[];
    if (!valid.length) return fetched;
    const merged = [...fetched, ...valid.filter(v => !have.has(v.id))].sort((a, b) => a.id - b.id);
    return merged;
  } catch {
    return fetched;
  }
